...
if ( $posts->have_posts() ) { ?>
	...
	<div class="<?php echo esc_attr( $attributes['className'] ); ?>">
		...
		<div class="cs-block-wide <?php echo esc_html( $attributes['layout'] ); ?>">
			...
			<div class="wide-outer">
				...
				while ( $posts->have_posts() ) {
					...
					// Check meta.
					$meta = csco_check_meta_of_wide_by( $attributes['layout'], $current );

					// Check excerpt.
					$excerpt = csco_check_excerpt_of_wide_by( $attributes['layout'], $current );

					// Check button.
					$button = csco_check_button_of_wide_by( $attributes['layout'], $current );

					// Get type.
					$type = csco_get_type_of_wide_by( $attributes['layout'], $current );

					// Get size.
					$size = csco_get_size_of_wide_by( $attributes['layout'], $current );

					// Get variation.
					$variation = csco_get_variation_of_wide_by( $attributes['layout'], $current );

					// Get orientation.
					$orientation = csco_get_orientation_of_wide_by( $attributes['layout'], $current );

					// Get typography.
					$typography = csco_get_typography_of_wide_by( $attributes['layout'], $current );
					...
					if ( isset( $params['rounded'] ) && $params['rounded'] ) {
						$post_class .= ' is-cnvs-block-rounded';
					}

					if ( has_post_thumbnail() ) {

						// Get required image size, depending on the wide type.
						$dependencies = csco_block_get_sizes_dependencies( 'wide' );
					...
					csco_before_hook_wide_by( $attributes['layout'], $current );
								...
								<?php if ( has_post_thumbnail() ) { ?>
									<div class="post-inner">
										<div class="post-thumbnail">
											...
											<?php if ( 'wide-6' === $attributes['layout'] || 'wide-11' === $attributes['layout'] ) { ?>
												<span class="post-number-wrap">
													<span class="post-number">
														<span><?php echo esc_html( $current ); ?> </span> <span><i class="cs-icon cs-icon-arrow-right"></i></span>
													</span>
												</span>
											<?php } ?>
					...
					csco_after_hook_wide_by( $attributes['layout'], $current );
				}
				?>
			</div>
